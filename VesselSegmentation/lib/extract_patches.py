...
def get_data_training(DRIVE_train_imgs_original,
    ...
    train_masks=np.zeros([1,4,train_imgs_original.shape[2],train_imgs_original.shape[3]])
    train_masks1 = load_hdf5(DRIVE_train_groudTruth +'1.hdf5')[img_id:img_id+1]#masks always the same
    train_masks2 = load_hdf5(DRIVE_train_groudTruth +'2.hdf5')[img_id:img_id+1] 
    train_masks3 = load_hdf5(DRIVE_train_groudTruth +'3.hdf5')[img_id:img_id+1] 
    train_masks4 = load_hdf5(DRIVE_train_groudTruth +'4.hdf5')[img_id:img_id+1] 
    print train_masks[:,0,:,:].shape
    train_masks[:,0,:,:]=train_masks1[:,0,:,:]
    train_masks[:,1,:,:]=train_masks2[:,0,:,:]
    train_masks[:,2,:,:]=train_masks3[:,0,:,:]
    train_masks[:,3,:,:]=train_masks4[:,0,:,:]
    # visualize(group_images(train_imgs_original[0:20,:,:,:],5),'imgs_train')#.show()  #check original imgs train

    print train_imgs_original[:,0,:,:].shape
    train_imgs = my_PreProc(train_imgs_original)
    train_imgs = train_imgs[img_id:img_id+1]
    ...
    print "train images range (min-max): " +str(np.min(train_imgs[:,0])) +' - '+str(np.max(train_imgs[:,0]))
    print "train images range (min-max): " +str(np.min(train_imgs[:,1])) +' - '+str(np.max(train_imgs[:,1]))
    print "train images range (min-max): " +str(np.min(train_imgs[:,2])) +' - '+str(np.max(train_imgs[:,2]))
...
def get_data_testing_overlap(DRIVE_test_imgs_original, DRIVE_test_groudTruth, Img_id, patch_height, patch_width, stride_height, stride_width):
    ### test
    test_imgs_original = load_hdf5(DRIVE_test_imgs_original)[Img_id:Img_id+1]
    test_masks = load_hdf5(DRIVE_test_groudTruth+'.hdf5')[Img_id:Img_id+1]
